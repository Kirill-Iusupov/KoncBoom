...
// Добавили image и brand для корректной типизации в корзине
export interface CartProduct {
  id: number | string;
  title: string;
  price: number;
  quantity: number;
  image: string;  // <-- Добавили фото
  brand?: string; // <-- Опционально добавили бренд (пригодится для UI)
}
...
const calculateTotals = (items: CartProduct[]) => {
  const totalQuantity = items.reduce((acc, item) => acc + item.quantity, 0);
  // Парсим цену на случай отрицательных значений с бэка
  const totalPrice = items.reduce((acc, item) => acc + (Math.abs(Number(item.price)) * item.quantity), 0);
  return { totalQuantity, totalPrice };
};
  ...
  persist(
    (set, get) => ({
      ...
      addToCart: (product) => {
        ...
        let newItems;
        if (existingItemIndex !== -1) {
          // Если товар уже есть — удаляем его (логика переключателя)
          newItems = items.filter((item) => item.id !== product.id);
        } else {
          // Добавляем новый товар со всеми его полями (включая image) + quantity: 1
          newItems = [...items, { ...product, quantity: 1 }];
        }
